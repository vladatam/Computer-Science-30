...
function Scribble(p) {
  ...
  this.buildEllipse = function( cx, cy, rx, ry, offset, overlap ) {
    let radialOffset = this.getOffset( -0.5, 0.5 )-Math.PI/2;
    ...
    for ( let theta = radialOffset; theta < Math.PI*2+radialOffset-0.01; theta+=this.ellipseInc ) {
  ...
  this.getIntersectingLines = function( lineCoords, xCoords, yCoords ) {
    let intersections = [];
    let s1 = new Segment( lineCoords[0], lineCoords[1], lineCoords[2], lineCoords[3] );

    for ( let i = 0; i < xCoords.length; i++ ) {
      let s2 = new Segment( xCoords[i], yCoords[i], xCoords[(i+1)%xCoords.length], yCoords[(i+1)%xCoords.length] );
  ...
  this.scribbleRect = function( x, y, w, h ) {
    let halfWidth = w/2;
    let halfHeight = h/2;
    let left   = Math.min( x-halfWidth, x+halfWidth );
    let right  = Math.max( x-halfWidth, x+halfWidth );
    let top    = Math.min( y-halfHeight, y+halfHeight );
    let bottom = Math.max( y-halfHeight, y+halfHeight );
  ...
  this.scribbleFilling = function( xCoords, yCoords, gap, angle ) {
    ...
    let hachureAngle = this.sketch.radians( angle%180 );
    let cosAngle = Math.cos( hachureAngle );
    let sinAngle = Math.sin( hachureAngle );
    ...
    let it = new HachureIterator( top-1, bottom+1, left-1, right+1, gap, sinAngle, cosAngle, tanAngle );
    let rectCoords = null;

    while ( (rectCoords = it.getNextLine()) != null ) {
      let lines = this.getIntersectingLines( rectCoords, xCoords, yCoords );
...
function HachureIterator( _top, _bottom, _left, _right, _gap, _sinAngle, _cosAngle, _tanAngle ) {
  let sinAngle = _sinAngle;
  let tanAngle = _tanAngle;
  let top = _top;
  let bottom = _bottom;
  let left = _left;
  let right = _right;
  ...
  this.getNextLine = function() {
  	if (Math.abs(sinAngle) < 0.0001) {
  		...
  		if (pos < right+deltaX)	{
  			...
  			let s = new Segment(xLower, yLower, xUpper, yUpper);
